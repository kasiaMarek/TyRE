...
mutual
  evidenceForRepetition : (re : CoreRE)
                      -> (mcvm  : (Maybe Char, VMState))
                      -> (accs  : List (w : Word ** Accepting (thompson re).nfa w))
                      -> (r : ExtendedRoutine)
                      -> (ford : r =
                              (accs >>= (\ac =>
                                        extractRoutine (thompson re).nfa (thompson re).prog (snd ac))))
                      -> (ev : Evidence **
                            (executeRoutineFrom r mcvm = (snd mcvm).evidence ++ ev,
  ...
  evidenceForRepetition re mcvm (acc :: accs) r ford with (trans ford (foldLeftIsConcatPrf accs acc _))
    evidenceForRepetition re mcvm (acc :: accs)
      ...
      let extr : ExtendedRoutine
          extr = extractRoutine (thompson re).nfa (thompson re).prog (snd acc)
          extrRest : ExtendedRoutine
          extrRest = (accs >>= (\ac => extractRoutine ((thompson re) .nfa) ((thompson re) .prog) (snd ac)))
          (ev ** (eq, enc)) := thompsonRoutinePrf re (snd acc) mcvm
          mcvm' : (Maybe Char, VMState)
          mcvm' = executeRoutineSteps extr mcvm
          (evsRest ** (eqEvs, encEvs)) := evidenceForRepetition re mcvm' accs extrRest Refl
      in (ev ++ evsRest **
          (rewrite (routineComposition extr extrRest mcvm) in
                      (trans eqEvs (trans (cong (++ evsRest) eq) (sym $ appendAssociative))),
             replace {p=((ev ++ evsRest) `Encodes`)}
                      (sym $ replicateForSucc (length accs) (Right $ ShapeCode re))
                        (recontextualise enc encEvs)))
  ...
  thompsonRoutinePrf Empty (Start ASt Here (Accept ASt Refl)) (mc, vm) = ([< UnitMark] ** (Refl, AnEmpty [<]))
  ...
  thompsonRoutinePrf (Star re) acc (mc, vm) =
    let (accs ** routineEq) := starEvidencePrf re acc
        (ev ** (evEq, enc)) := evidenceForRepetition re (executeRoutineSteps [Regular EmitEList] (mc, vm)) accs
                                (accs >>= (\ac => extractRoutine (thompson re).nfa (thompson re).prog (snd ac)))
                                Refl
    in ([< EList] ++ ev :< BList **
        (rewrite routineEq in (trans  (repRoutinePrf _ _)
                                      (cong (:< BList) (trans evEq (sym $ appendAssociative)))),
        ARepetiton [<] enc))
